...
| **Tavily as a secondary news provider** (relevance-ranked web search alongside Finnhub's aggregator feed) | The News agent or CIO demonstrably misses a market-moving story that a web search would have caught, AND the Tavily free quota (1,000 credits/mo) is no longer burned by the heartbeat — i.e. only the drill-in would call it. Finnhub replaced Tavily outright on 2026-09-08 (ARCHITECTURE §3.11). | ~1h: restore `data/tavily.py` from git history behind the same `search_news` signature and merge the two lists by URL. |
