...
| **20-F + 6-K (foreign issuer reports)** — annual + interim filings for non-US-domiciled tickers. **Currently affects TSM (Taiwan Semiconductor) and ASML (Netherlands)** — both AI cake universe tickers that have ZERO chunks in ChromaDB because `data/edgar.DEFAULT_LIMITS = {"10-K": 2, "10-Q": 4}` doesn't match their filings. Drill-ins on TSM or ASML will run with empty Filings output. | Material gap discovered late Step 8 catch-up ingestion. The dashboard's ingest banner will mis-prompt the user to run `scripts.ingest_universe TSM` (which won't help — it'll still find zero filings in our supported kinds). | Add `"20-F"` + `"6-K"` to `data/edgar.DEFAULT_LIMITS`, re-test the parser on a sample 20-F (different document structure than 10-K), re-ingest. ~1h. Until then: TSM + ASML produce thesis-incomplete reports — Filings agent emits a foreign-issuer aware error so the dashboard surfaces the real reason. |
